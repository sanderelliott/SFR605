...
```{r}
fav1 <- lmer(Z_TP ~ Z_Length + (1 + Z_Length | Fish_Species) + (1 | Lake),
           data = fish.data, REML = TRUE)

fav2 <- lmer(Z_TP ~ Z_Length + (1 + Z_Length | Fish_Species) + (1 | Lake),
           data = fish.data, REML = FALSE)

fav_pql <- glmmPQL(
  Z_TP ~ Z_Length,
  random = list(
    Fish_Species = pdSymm(~ Z_Length),
    Lake = ~ 1
  ),
  family = gaussian,
  data = fish.data
