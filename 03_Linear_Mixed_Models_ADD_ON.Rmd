...
By species, It looks like the intercept changes, ascending order:  1,3,2. The slope may also change. Species 1 looks like it may have a lower slope than the other two. By lake it is a bit harder to tell, but it does look like the intercept varies. 
...
# Full model with varying intercepts #Hint: you already defined this above
M1 <- lmer(Z_TP ~ Z_Length + (1 | Lake) + (1 | Fish_Species),
     data = fish.data, REML = FALSE)
# Full model with varying intercepts and slopes #Hint: you already defined this above
M2 <- lmer(Z_TP ~ Z_Length + (1 + Z_Length | Lake) + (1 + Z_Length | Fish_Species),
     data = fish.data, REML = FALSE)
# No Lake, varying intercepts only
M3 <- lmer(Z_TP ~ Z_Length + (1 | Fish_Species),
           data = fish.data, REML = FALSE)
# No Species, varying intercepts only
M4 <- lmer(Z_TP ~ Z_Length + (1 | Lake),
           data = fish.data, REML = FALSE)
...
M6 <- lmer(Z_TP ~ Z_Length + (1 + Z_Length | Lake),
     data = fish.data, REML = FALSE)
# Full model with varying intercepts, and slopes only varying by lake
M7 <- lmer(Z_TP ~ Z_Length + (1 + Z_Length | Lake) + (1 | Fish_Species),
     data = fish.data, REML = FALSE)
...
M8 <- lmer(Z_TP ~ Z_Length + (1 + Z_Length | Fish_Species) + (1 | Lake),
           data = fish.data, REML = TRUE)
AIC.table  <- MuMIn::model.sel(M2, M8)
...
Slope = 0.4222697.
Confidence interval = 0.241543 - 0.6029964

Is the slope of Z_Length significantly different from 0?
Yes, 0 is not inside the confidence interval.
...
```{r}
CIl <- 0.4222697 - 1.96 * 0.0922075
CIu <- 0.4222697 + 1.96 * 0.0922075
CIl
CIu
```
...
head(d)
head(fish.data)
ggplot(data = fish.data, aes(x = Z_Length, y = Z_TP, colour = Lake)) +
  geom_point() +
  geom_abline(data = d,
              aes(intercept = `(Intercept)`, slope = Z_Length, colour = Lake )) +
  theme_classic()
...
head(d)
head(fish.data)
ggplot(data = fish.data, aes(x = Z_Length, y = Z_TP, colour = Fish_Species)) +
  geom_point() +
  geom_abline(data = d,
              aes(intercept = `(Intercept)`, slope = Z_Length, colour = Fish_Species )) +
  theme_classic()
```
...
# Extension for Week - day - Assignment


```{r}
fav <- lmer(Z_TP ~ Z_Length + (1 + Z_Length | Fish_Species) + (1 | Lake),
           data = fish.data, REML = FALSE)
fav

fam_reml <- lmer(Z_TP ~ Z_Length + (1 + Z_Length | Fish_Species) + (1 | Lake),
           data = fish.data, REML = TRUE)
fam_reml
...
fav_pql <- glmmPQL(
  ...
  random = list(
    Lake = ~ 1,
    Fish_Species = ~ Z_Length
  ),
  family = gaussian,
  data = fish.data)
fav_pql

AICfav  <- MuMIn::model.sel(fav, fam_reml, fav_pql)
AICfav
...
1)
The PQL does not have any comparative statistics. The maximum likelihood model had a smaller AIC value than the REML and is likely the stronger of the two. 

2)
The marginal R2 is much lower than the conditional, so the random effects, species and length, acount for much more of the variation than the fixed effect of length. 
