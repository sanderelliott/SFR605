---
title: "Tutorial - Hierarchical Bayes"
author: "SFR 605"
output: html_document
---

The objective of this tutorial is to explore basic Bayesian hierarchical models.  We will focus on the most common class of hierarchical models, which are linear mixed models.  Mixed models refer to models that include both hierarchical “random” effects and non-hierarchical “fixed” effects.  Everything that we apply below to linear models can also be applied to generalized linear models (e.g. logistic and poisson regression) and thus generalized linear mixed models for which all of our traditional non-hierarchical linear and GLM exist as a special case.  While we have focused on random effects from Maximum Likelihood perspective earlier in the course, we will now approach the same model structure using the Bayesian perspective, where we can easily relax additional assumptions.
...
2.	Fit a Bayesian model for the overall "global" mean `mu`, and precision `tau` (1/(sd^2)), reporting summary statistics for both. 
...
You are discussing your research with a colleague and mention that your random effects model showed that one year, 2002, had notably lower mosquito abundance.  He suggests that the driver may be exogenous and sends you a data file, `met.csv` (you will find this data in the `SFR605/data` folder), that contains the mean annual temperature (°C), precipitation (mm/year), and relative humidity (%) for 1995-2009 years.
...
7.	Convert the random effects model to a mixed effects model by converting the mean, mu, to a linear model, `beta0 + beta1*met[i]` where met is the meteorological covariate you want to include, while keeping the random year effect.
...
10.	Optional: Use the best fitting model to predict the next 5 years (2005-2009) of mosquito abundance including an uncertainty estimate (predictive interval). Turn in a graph of your prediction. Hint: the easiest way to make predictions is to create new rows in your data object that has covariates but NA's for the y's.
