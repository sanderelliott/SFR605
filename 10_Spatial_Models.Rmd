...
```{r}
surf1 <- surf.ls(1,dat$Lon,dat$Lat,dat$Mean)   ## 0-degree polynomial surface
summary(surf1)                                 ## ANOVA stats & AIC
tr1 <- trmat(surf1,-125,-65,25,50,50)          ## project a 50x50 matrix within region
image(tr1,asp=70/50)                           ## make an color image map of the surface
map("state",add=TRUE)
points(dat$Lon,dat$Lat,pch="+",col=dat$Mean/10)

surf2 <- surf.ls(2,dat$Lon,dat$Lat,dat$Mean)   ## 0-degree polynomial surface
summary(surf2)                                 ## ANOVA stats & AIC
tr2 <- trmat(surf2,-125,-65,25,50,50)          ## project a 50x50 matrix within region
image(tr2,asp=70/50)                           ## make an color image map of the surface
map("state",add=TRUE)
points(dat$Lon,dat$Lat,pch="+",col=dat$Mean/10)
...
I am choosing a second order polynomial as finer resolution scale. This allows it to show a better pattern. 
...
```{r}
vg <- variogram(surf2,300,xlim=c(0,10),ylim=c(0,1.1*var(dat$Mean)))

abline(h=var(dat$Mean))                   ## asymptotic variance

cg<- correlogram(surf2,300,xlim=c(0,10))  ## limit the range of radii to the initial decay
```
...
```{r}
par(lwd=2,cex=1.1)
cg <- correlogram(surf2,300,xlim=c(0,10))
...
```{r}
                 ## define the distance bins we want to use for the fit
...
sphfit <- function(theta){    ## negative log likelihood function
  ...
  se = theta[3]
  sigma = theta[4]
  -sum(dnorm(cg$y[rng],sphercov(cg$x[rng],d,alpha,se),sigma,log=TRUE)) 
}
sfit <- optim(c(2,0.1,1,0.01),sphfit)    ## numerical optimization
sfit
...
E.AIC <- 2*4 - 2*efit$value
S.AIC <- 2*4 - 2*sfit$value
G.AIC <- 2*4 - 2*gfit$value
...
This figure shoes the correllogram for all three models. All three models follow a similar pattern. This indicates that one covariance stricture is not too much better than another. It looks like spherical might be slightly better, but again, the difference is small. The range is 10, the number of distance bins we want to fit. 
...
```{r}
KrExp <- surf.gls(2,sphercov,dat$Lon,dat$Lat,dat$Mean,
...
*Map above*. Highest ozone levels seem to be between 34 and 43 degrees latitude, and with a western bias. Locally high ozone levels are present around cities, especially in the rockies. 
...
There is a lot of uncertainty. Mostly in areas farther from places of measurement which makes sense. 
...
One could explore different interpolation surfaces. Also it may be worth trying higher order polynomials (>2) and see if it improves the model using the same steps done in this tutorial.  
